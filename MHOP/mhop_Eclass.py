...
#from reque import ReQueue
from mh_edit_class import MinimaHoppingWithTempControl,MHPlot
...
# Identifying T0,maxtemp, (Kelvin)
T0 = 300.0
T_max = 1500

# Run minima hopping algorithm

mh = MinimaHoppingWithTempControl(
    slab,
    T0=T0,  # initial MD "temperature" (K)
    T_max=T_max, # temperature limitation
    Ediff0=1.0,  # initial acceptance window (eV)
    mdmin=10,  # MD stop criterion (No. of minima)
    logfile='hop.log',
    minima_traj='minima.traj',
    timestep = 0.2,  # MD time step (fs)
)
