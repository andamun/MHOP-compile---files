# Modify the MinimaHopping class obtain the code from https://github.com/PaNOSC-ViNYL/ase/blob/master/ase/optimize/minimahopping.py
...
class MinimaHoppingWithTempControl:
    ...
    _default_settings = {
        ...
        'T_max':None,  # K, maximum allowed temperature during MD
    ...
    def __init__(self, atoms,T_max=None, **kwargs):
        """
        Parameters
        ----------
        atoms : Atoms object
        ...
        max_temp : float, optional
            Maximum allowed temperature during MD in Kelvin.
            If None, no temperature limiting is applied.
        temp_scale_factor : float, optional
            Factor to multiply velocities by when max_temp is exceeded.
            Default is 0.9 (reduces temperature by ~19%)
        **kwargs : 
            Additional parameters passed to original MinimaHopping
        """
    ...
    def _check_results(self):
        ...
        # --- HELPER: RESET TEMP IF TOO HIGH ---
        def check_and_reset_temp(const=1.0):
            # Check if T_max is set and if we exceeded it
            if self.T_max is not None and self._temperature > self.T_max:
                self._log('msg', 'Temperature (%.2f K) exceeded limit (%.2f K).' % 
                          (self._temperature, self.T_max))
                self._temperature = self._T0 * const
                self._log('msg', 'Resetting Temperature to T0 (%.2f K).' % self._temperature)

        # 1. No prior minima found?
        self._read_minima()
        ...
        # 2. Returned to starting position?
        if self._previous_optimum:
            compare = ComparePositions(translate=False) # Calling class object
            dmax = compare(self._atoms, self._previous_optimum) # Compare between the previous optimum and current position
            self._log('msg', 'Max distance to last minimum: %.3f A' % dmax)
            if dmax < self._minima_threshold: # case for no change in position
                self._log('msg', 'Re-found last minimum.')
                beta1 = np.random.uniform(1.01,1.20)
                beta1 = round(beta1, 2)
                self._temperature *= round(beta1, 2) # Increase Temp
                
                # <--- INSERT FIX HERE: Check limit BEFORE logging/returning
                const=np.random.uniform(1.0,1.5)
                check_and_reset_temp(const=round(const,2)) 
                
                self._log('par')
                return

        # 3. In a previously found position?
        unique, dmax_closest = self._unique_minimum_position() # This will determine whether the finding is unique or not
        self._log('msg', 'Max distance to closest minimum: %.3f A' % dmax_closest)
        if not unique:
            beta2 = np.random.uniform(1.01,1.20)
            beta2 = round(beta2,2)
            self._temperature *= beta2 # Increase Temp
        ...
        # 4. Must have found a unique minimum.
        beta3 = np.random.uniform(0.9,1.0) #lower temperature a bit after minima
        beta3 = round(beta3,2)
        self._temperature *= round(beta3,2)
        
        self._log('msg', 'Found a new minimum.')
        self._log('par')
        if (self._atoms.get_potential_energy() < self._previous_energy + self._Ediff):
            self._log('msg', 'Accepted new minimum.')
    ...
    def _molecular_dynamics(self, resume=None):
        ...
        if not thermalized:
            '''
            # Find current temperauture
            current_temp = self._atoms.get_temperature() # in Kelvin unit
            if self._T_max is not None:
                if current_temp > self._T_max:
                    self._temperature = self._T_max * np.random.rand()
                else:
        ...
        while mincount < self._mdmin:
            ...
            if self._T_max is not None:
                ...
                if current_temp > self.T_max:
                    new_temp = self._T_max * np.random.rand()
                    MaxwellBoltzmannDistribution(
                self._atoms,
                temp=new_temp * units.kB,
                force_temp=True
            )
                self._log('msg', f'T={current_temp:.1f}K exceeded T_max, reset to {new_temp:.1f}K')
