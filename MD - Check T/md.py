...
def apply_pcnb_constraints_universal(atoms,slab_atom,length_atom_slab):
    ...
    for mol_id, cluster_indices in enumerate(molecular_clusters):
        ...
        carbons = [item for item in cluster_atoms if item[1].symbol == 'S']
...
mask = [i for i in range(0,int(len(cu_indices)),1)]
...
dyn.run(10000) # running for 25 PS
